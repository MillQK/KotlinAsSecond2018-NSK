...
fun lineBySegment(s: Segment): Line {
    val xLen = Math.abs(s.begin.x - s.end.x)
    val yLen = Math.abs(s.begin.y - s.end.y)
    val angle = Math.atan2(yLen, xLen)
...
fun lineByPoints(a: Point, b: Point): Line {
    val xLen = Math.abs(a.x - b.x)
    val yLen = Math.abs(a.y - b.y)
    val angle = Math.atan2(yLen, xLen)
    return Line(a, angle)
}
...
fun findNearestCirclePair(vararg circles: Circle): Pair<Circle, Circle> {
    ...
    var resultPair = Pair(circles[0], circles[0])
